...
1. Ти ОБОВ'ЯЗКОВО повинен використати хоча б один інструмент верифікації (capture_screen, analyze_screen, read_file, run_shell) ПЕРЕД тим, як звітувати про невизначеність.
2. Якщо у повідомленні Тетяни є помилка tool execution ("status": "error") — це ЗАВЖДИ FAILED, НЕ uncertain!
3. Якщо tool результат показує успіх — це SUCCESS.
4. Тільки якщо після використання інструментів ти все ще не впевнений — можна звітувати про невизначеність.
