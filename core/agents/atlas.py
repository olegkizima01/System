...
Твоя роль: Визначити оптимальну політику виконання (Execution Policy) для поточної ситуації.

Твої обов'язки:
1. Аналізуй успіх/проввал останнього кроку.
2. Встановлюй рівень верифікації (rigor):
   - 'high': якщо крок критичний або були помилки.
   - 'low': для рутинних, перевірених дій.
3. Вибирай Recovery Strategy:
   - 'local_fix': спробувати виправити конкретний крок.
   - 'full_replan': якщо стратегія застаріла або змінилися умови.
4. Визначай Tool Preference: (native, gui, hybrid).

Обґрунтуй своє рішення у полі 'reasoning'.
"""
...
🧠 ЕТАПИ МИСЛЕННЯ:
1. Аналіз цілі та контексту.
2. Формування Meta-Config (Політика).
3. Побудова кроків.
4. SELF-REVIEW: Чи безпечний цей план? Чи не веде він до циклу? Чи достатньо верифікації?

Правила планування:
1. Кроки мають бути ДІЄВИМИ (Tools calls).
2. КАТЕГОРИЧНО ЗАБОРОНЕНО: Умовні переходи "якщо" в описі.
3. ПРІОРИТЕТ: На google.com без результатів -> першим кроком ПОШУК.
4. ОБОВ'ЯЗКОВО: Після критичного кроку — крок "verify" для Grisha.
