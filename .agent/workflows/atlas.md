...
graph TD
    ...
    MP -->|потрібен план| A[atlas<br/>Архітектор Плану]
    MP -->|план готовий| T[tetyana<br/>Виконавець]
    MP -->|план готовий| G[grisha<br/>Верифікатор]
    A --> MP
    T --> G
    G --> MP
    MP -->|завершено| END((END))
```

### 2.1 Trinity Agents

-   **Meta-Planner** (`_meta_planner_node`): Встановлює **Execution Policy** (стратегічне обґрунтування через `reasoning`). Вирішує, чи ми рухаємося далі, чи потрібне локальне виправлення (`repair`), чи глобальне перепланування. Також відповідає за:
    -   **Summarization**: Кожні 3 кроки стискає історію в `summary`.
    -   **Step Consumption**: Видаляє виконані кроки з плану після успіху Grisha.
    -   **Tool Preference**: Визначає пріоритет інструментів (native/gui/hybrid).
    -   **Limit Checks**: Контролює ліміти (50 кроків, 10 перепланувань).
-   **Atlas** (`_atlas_node`): Перетворює політику на кроки. Включає фазу **Self-Review** (перевірка плану на цикли та безпеку перед видачею). Активує **Adaptive Verifier**.
-   **Tetyana** (`_tetyana_node`): Виконує команди. Має механізм анти-циклів та Hybrid Fallback.
-   **Grisha** (`_grisha_node`): Валідатор. Читає екран, аналізує UI та дає вердикт: `success`, `failed` або `uncertain`.
